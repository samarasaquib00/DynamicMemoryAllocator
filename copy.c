...
static inline header *split_block(header *block, int allocable_size) {
  ...
  header *new_block = (header *)((char*)block + extra_block_allocable_size + ALLOC_HEADER_SIZE);

  set_size(new_block, allocable_size + ALLOC_HEADER_SIZE);
  set_size(block, extra_block_allocable_size + ALLOC_HEADER_SIZE);

  set_state(new_block, ALLOCATED);
  set_state(block, UNALLOCATED);

  new_block->left_size = get_size(block);
  insert_block(block);
  ...
  if (block == NULL) {
    ...
    if ( (get_header_from_offset(new_fp1, -ALLOC_HEADER_SIZE)) == get_header_from_offset(lastFencePost, 0) ) {
      ...
      if (get_state(left) == UNALLOCATED) {
        ...
        //lastFencePost = (header*) ((char*) left + get_size(left));
      } else {  /* left block is allcated */
        /* Move "new" back to lastFencePost's location */
        //new = ( (header *) new-(2*ALLOC_HEADER_SIZE) );
        new = get_header_from_offset(new, -(2*ALLOC_HEADER_SIZE));
        set_size(new, get_size(new) + (2*ALLOC_HEADER_SIZE) );
        set_state(new, UNALLOCATED);
        //insert_block(new);
    ...
    return (header *)allocate_object(raw_size);    // use recursion --> call alllocate object again
  }
